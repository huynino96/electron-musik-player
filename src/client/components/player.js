Vue.component('player', {
  ...
  computed: {
    ...
    playTime: function () {
      return (
        (this.player.interface.info.currentTime /
          this.player.interface.info.duration) *
        100
      );
    },
    playIndex: function () {
      ...
      if (
        ...
        this.selected.type == 'playlist' &&
        this.player.playing
      ) {
        for (let [i, v] of this.selected.content.entries())
          if (v.id == this.player.playing.id) return i;
      }
    ...
    theme: function () {
      return (
        this.player.profile.data.theme + ' ' + this.player.profile.data.skin
      );
    },
  },
  methods: {
    ...
    seekTo: function (e) {
      if (e.type != 'mousemove' || e.buttons == 1) {
        ...
        this.player.interface.seek(
          (xPos / track.width()) * this.player.interface.info.duration
        );
    ...
    updateColor: function () {
      document.documentElement.style.setProperty(
        '--primary-color',
        this.player.profile.data.color
      );
      document.documentElement.style.setProperty(
        '--primary-hover',
        this.lighten(this.player.profile.data.color, -20)
      );
      document.documentElement.style.setProperty(
        '--primary-track',
        this.lighten(this.player.profile.data.color, 20)
